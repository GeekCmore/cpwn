...
def get_glibc_files(version: str, arch: str) -> dict:
    ...
    if float(version[0:4]) >= 2.39:
        glibc_files[BaseFile.LIBC] = os.path.join(
            expect_dir,
            f"{arch}/libc6_{version}_{arch}/usr/lib/x86_64-linux-gnu/libc.so.6",
        )
        glibc_files[BaseFile.LD] = os.path.join(
            expect_dir,
            f"{arch}/libc6_{version}_{arch}/usr/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2",
        )
    else:
        glibc_files[BaseFile.LIBC] = os.path.join(
            expect_dir, f"{arch}/libc6_{version}_{arch}/lib/x86_64-linux-gnu/libc.so.6"
        )
        glibc_files[BaseFile.LD] = os.path.join(
            expect_dir,
            f"{arch}/libc6_{version}_{arch}/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2",
        )
